#ifndef INC__JACURSES_TERM_H
#define INC__JACURSES_TERM_H
...
/* rxvt-256color */
static const char* rxvt_256color_keys[]  = { "\033[11~",
        "\033[12~",
        "\033[13~",
        "\033[14~",
        "\033[15~",
...
/* Eterm */
static const char* eterm_keys[]  = { "\033[11~",
        "\033[12~",
        "\033[13~",
        "\033[14~",
        "\033[15~",
...
/* screen */
static const char* screen_keys[]  = { "\033OP",
        "\033OQ",
        "\033OR",
        "\033OS",
        "\033[15~",
...
/* rxvt-unicode */
static const char* rxvt_unicode_keys[]  = { "\033[11~",
        "\033[12~",
        "\033[13~",
        "\033[14~",
        "\033[15~",
...
/* linux */
static const char* linux_keys[]  = { "\033[[A",
        "\033[[B",
        "\033[[C",
        "\033[[D",
        "\033[[E",
...
/* xterm */
static const char* xterm_keys[]  = { "\033OP",
        "\033OQ",
        "\033OR",
        "\033OS",
        "\033[15~",
...
static int try_compatible( const char* term,
	const char* name,
	const char** tkeys,
	const char** tfuncs )
{
...
static int init_term_builtin( void )
{
	...
	if( term )
	{
		...
		if( try_compatible( term, "xterm", xterm_keys, xterm_funcs ) ==
			0 )
			return 0;
		if( try_compatible( term,
			    "rxvt",
			    rxvt_unicode_keys,
			    rxvt_unicode_funcs ) == 0 )
			return 0;
		if( try_compatible( term, "linux", linux_keys, linux_funcs ) ==
			0 )
			return 0;
		if( try_compatible( term, "Eterm", eterm_keys, eterm_funcs ) ==
			0 )
			return 0;
		if( try_compatible(
			    term, "screen", screen_keys, screen_funcs ) == 0 )
			return 0;
		if( try_compatible(
			    term, "tmux", screen_keys, screen_funcs ) == 0 )
			return 0;
		/* let's assume that 'cygwin' is xterm compatible */
		if( try_compatible(
			    term, "cygwin", xterm_keys, xterm_funcs ) == 0 )
			return 0;
	}
...
static char* load_terminfo( void )
{
	...
	if( home )
	{
		...
		tmp[sizeof( tmp ) - 1] = '\0';
		data                   = terminfo_try_path( tmp, term );
		if( data )
			return data;
	}
	...
	/* next, TERMINFO_DIRS */
	{
		...
		if( dirs )
		{
			...
			while( dir )
			{
				...
				data = terminfo_try_path( cdir, term );
				if( data )
					return data;
				dir = strtok( 0, ":" );
			}
...
static int init_term( void )
{
	...
	header = (int16_t*)data;
	{
		...
		{
			const int str_offset = TI_HEADER_LENGTH + header[1] +
				header[2] + number_sec_len * header[3];
			const int table_offset = str_offset + 2 * header[4];

			keys = malloc(
				sizeof( const char* ) * ( TB_KEYS_NUM + 1 ) );
			for( i = 0; i < TB_KEYS_NUM; i++ )
			{
				keys[i] = terminfo_copy_string( data,
					str_offset + 2 * ti_keys[i],
					table_offset );
			}
			keys[TB_KEYS_NUM] = 0;

			funcs = malloc( sizeof( const char* ) * T_FUNCS_NUM );
			/* the last two entries are reserved for mouse. because
			 * the table
			 * offset is not there, the two entries have to fill in
			 * manually */
			for( i = 0; i < T_FUNCS_NUM - 2; i++ )
			{
				funcs[i] = terminfo_copy_string( data,
					str_offset + 2 * ti_funcs[i],
					table_offset );
			}
...
static void shutdown_term( void )
{
	if( init_from_terminfo )
	{
		...
		/* the last two entries are reserved for mouse. because the
		 * table offset is not there, the two entries have to fill in
