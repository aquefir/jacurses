...
#ifndef INC__JACURSES_INPUT_H
#define INC__JACURSES_INPUT_H
...
static int parse_mouse_event(
	struct tb_event* event, const char* buf, int len )
{
	if( len >= 6 && starts_with( buf, len, "\033[M" ) )
	{
		...
		event->x = (u8)buf[4] - 1 - 32;
		event->y = (u8)buf[5] - 1 - 32;
...
static bool extract_event(
	struct tb_event* event, struct bytebuffer* inbuf, int inputmode )
{
		...
		else
		{
			...
			else if( inputmode & TB_INPUT_ALT )
			{
				...
				return extract_event(
					event, inbuf, inputmode );
			}
			assert( !"never got here" );
		}
	}

	/* if we're here, this is not an escape sequence and not an alt
	 * sequence so, it's a FUNCTIONAL KEY or a UNICODE character */
