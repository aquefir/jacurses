...
static u16 background = TB_DEFAULT;
static u16 foreground = TB_DEFAULT;

static void write_cursor( int x, int y );
static void write_sgr( u16 fg, u16 bg );
...
static void send_attr( u16 fg, u16 bg );
static void send_char( int x, int y, u32 c );
...
int tb_init_fd( int inout_ )
{
	...
	}
	{
		...
		tios.c_iflag &= ~( IGNBRK | BRKINT | PARMRK | ISTRIP | INLCR |
			IGNCR | ICRNL | IXON );
		tios.c_oflag &= ~OPOST;
...
void tb_present( void )
{
	...
	for( y = 0; y < front_buffer.height; ++y )
	{
		for( x = 0; x < front_buffer.width; )
		{
			...
			if( w < 1 )
				w = 1;
			if( memcmp( back, front, sizeof( struct tb_cell ) ) ==
				0 )
			{
				x += w;
				continue;
...
void tb_change_cell( int x, int y, u32 ch, u16 fg, u16 bg )
{
	struct tb_cell c;
	c.ch = ch;
	c.fg = fg;
...
struct tb_cell* tb_cell_buffer( void ) { return back_buffer.cells; }
...
int tb_select_input_mode( int mode )
{
	if( mode )
	{
		...
		/* technically termbox can handle that, but let's be nice and
		   show here what mode is actually used */
		...
		{
			bytebuffer_puts(
				&output_buffer, funcs[T_ENTER_MOUSE] );
			bytebuffer_flush( &output_buffer, inout );
		}
		else
		{
...
void tb_set_clear_attributes( u16 fg, u16 bg )
{
	foreground = fg;
	background = bg;
}
...
static int convertnum( u32 num, char* buf )
{
	int i, l = 0;
	int ch;
	do
...
#define WRITE_LITERAL( X ) \
	bytebuffer_append( &output_buffer, ( X ), sizeof( X ) - 1 )
#define WRITE_INT( X ) \
...
static void cellbuf_resize( struct cellbuf* buf, int width, int height )
{
	...
	minw = ( width < oldw ) ? width : oldw;
	minh = ( height < oldh ) ? height : oldh;
...
#define LAST_ATTR_INIT 0xFFFF
	static u16 lastfg = LAST_ATTR_INIT, lastbg = LAST_ATTR_INIT;
...
static int read_up_to( int n )
{
	...
	while( read_n <= n )
	{
		...
		{
			r = read( inout,
				input_buffer.buf + prevlen + read_n,
				n - read_n );
		}
#ifdef __CYGWIN__
		/* While linux man for tty says when VMIN == 0 && VTIME == 0,
		 */
		/* read should return 0 when there is nothing to read, cygwin's
		 */
		/* read returns -1. Not sure why and if it's correct to ignore
		 */
		/* it, but let's pretend it's zero. */
		if( r < 0 )
			r = 0;
#endif
		if( r < 0 )
...
#define ENOUGH_DATA_FOR_PARSING 64
	...
	while( 1 )
	{
		...
		maxfd  = ( winch_fds[0] > inout ) ? winch_fds[0] : inout;
